...
def is_job_question(msg):
    ...
    keywords = [
        "job", "jobs", "career", "skills",
        "company", "hire", "hiring", "role",
        "recommend", "latest"
    ]
...
def insert_default_jobs():
    ...
    if count == 0:
        ...
        default_jobs = [
            ("ABC Corp", "java developer", "java,spring,sql", "default", "2026-05-20"),
            ("Google", "data scientist", "python,machine learning,pandas", "default", "2026-05-10"),
            ("Amazon", "data engineer", "python,sql,spark,aws", "default", "2026-04-15"),
            ("Microsoft", "data analyst", "excel,sql,power bi", "default", "2026-03-01"),
            ("Meta", "ml engineer", "python,machine learning,tensorflow", "default", "2025-12-15"),
            # =========================================
            # NOISY DATA
            # =========================================
        ...
        cursor.executemany("""
            INSERT INTO jobs (company, role, skills, job_type, posted_date)
            VALUES (?, ?, ?, ?, ?)
        """, default_jobs)
...
def job_portal(user_role, user_skills):
    ...
    cursor.execute("SELECT company, role, skills, posted_date FROM jobs")
    data = cursor.fetchall()
    conn.close()

    for row in data:

        company, role, skills_str, posted_date = row
        ...
        # USER MATCH
        # ===========================
        level = "Strong Match" if score >= 70 else "Moderate Match" if score >= 40 else "Weak Match"
